...
export default function SignupPage() {
  ...
  async function handleGoogle() {
    ...
    if (error) {
      setError('Google sign-up is not available for this workspace yet.')
      setLoading(false)
    }
  }
        ...
        <div className="card">
          <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <Field label="Organization name">
              <input className="input-field" placeholder="Your company" value={orgName} onChange={e => setOrgName(e.target.value)} required />
            </Field>
            ...
            <Field label="Password">
              <input className="input-field" type="password" placeholder="At least 8 characters" value={password} onChange={e => setPassword(e.target.value)} minLength={8} required />
            </Field>
          ...
          {GOOGLE_AUTH_ENABLED && (
            <>
              ...
              <button onClick={handleGoogle} className="btn-ghost" disabled={loading} style={{ width: '100%', justifyContent: 'center' }}>
                <GoogleIcon />
